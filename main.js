...
/* ═══ LANGUAGE PREFERENCE — persistenza scelta utente ═══
   Prima visita → sempre IT (default)
   Se utente clicca EN → ricorda EN
   Se utente clicca IT → ricorda IT (torna a default)
   ═══════════════════════════════════════════════════════ */
(function(){
  const LANG_KEY = 'dg_lang';
  const isEN = location.pathname.startsWith('/en') || location.pathname.includes('/en/');
  const isIT = !isEN;
  
  // Registra la scelta quando l'utente naviga tra lingue
  if(isEN) {
    localStorage.setItem(LANG_KEY, 'en');
  } else {
    // Se siamo in IT, è perché l'utente è venuto qui esplicitamente
    // Non sovrascrivere se è la prima visita
    if(!localStorage.getItem(LANG_KEY)) {
      localStorage.setItem(LANG_KEY, 'it');
    }
  }
  
  // Click su IT toggle in EN page → imposta IT, vai alla home IT
  document.querySelectorAll('a[hreflang="it"]').forEach(a => {
    a.addEventListener('click', () => {
      localStorage.setItem(LANG_KEY, 'it');
    });
  });
  
  // Click su EN toggle in IT page → imposta EN
  document.querySelectorAll('a[hreflang="en"]').forEach(a => {
    a.addEventListener('click', () => {
      localStorage.setItem(LANG_KEY, 'en');
    });
  });
